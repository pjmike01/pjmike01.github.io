---
title: "springboot 整合 Netty 实战"
date: 2018-10-28
slug: "springboot-整合-Netty-实战"
tags: ["Netty", "springboot"]
---
**Catalogue**

1.  [1. 前言](#前言)
2.  [2. 构建 Netty 服务端](#构建-Netty-服务端)
3.  [3. 构建 Netty 客户端](#构建-Netty-客户端)
4.  [4. 使用 protobuf 构建通信协议](#使用-protobuf-构建通信协议)
    1.  [4.1. protobuf简介](#protobuf简介)
    2.  [4.2. 为什么要用protobuf](#为什么要用protobuf)
    3.  [4.3. 怎么使用protobuf](#怎么使用protobuf)
        1.  [4.3.1. 定义 protobuf 协议格式](#定义-protobuf-协议格式)
        2.  [4.3.2. 使用 .proto编译器编译](#使用-proto编译器编译)
        3.  [4.3.3. 使用 Java 对应 的 protobuf API来读写消息](#使用-Java-对应-的-protobuf-API来读写消息)
    4.  [4.4. protobuf的编解码器](#protobuf的编解码器)
5.  [5. 客户端心跳机制](#客户端心跳机制)
    1.  [5.1. 心跳机制简介](#心跳机制简介)
    2.  [5.2. 如何实现心跳机制](#如何实现心跳机制)
    3.  [5.3. Netty 实现心跳机制](#Netty-实现心跳机制)
6.  [6. Netty 客户端断线重连](#Netty-客户端断线重连)
7.  [7. 服务端空闲检测](#服务端空闲检测)
8.  [8. Controller方法测试](#Controller方法测试)
9.  [9. 小结](#小结)
...
*   利用protobuf定义消息格式
*   服务端空闲检测
*   客户端发送心跳包与断线重连 PS: 我这里为了简单起见（主要是懒），将 Netty 服务端与客户端放在了同一个SpringBoot工程里，当然也可以将客户端和服务端分开。
...
上面简单的介绍了 protobuf的一些语法规则，关于 protobuf语法的更多介绍参考官方文档：[https://developers.google.com/protocol-buffers/docs/proto3](https://developers.google.com/protocol-buffers/docs/proto3)

#### 使用 `.proto`编译器编译

第一步已经定义好了 protobuf的消息格式，然后我们用 `.proto`文件的编译器将我们定义的 消息格式编译生成对应的 Java类，以便于我们在项目中使用该消息类。

关于protobuf编译器的安装这里我就不细说，详情见官方文档： [https://developers.google.com/protocol-buffers/](https://developers.google.com/protocol-buffers/)
...
这里就不多介绍protobuf-java API的相关用法了，更多详情还是参考官方文档：[https://developers.google.com/protocol-buffers/docs/reference/java/](https://developers.google.com/protocol-buffers/docs/reference/java/)
...
上面详细阐述了 如何用 SpringBoot 整合 Netty ,其中借鉴很多前辈大佬的例子与文章，算是初步了解了如何使用 Netty。上文中如有错误之处，欢迎指出。github地址： [https://github.com/pjmike/springboot-netty](https://github.com/pjmike/springboot-netty)
