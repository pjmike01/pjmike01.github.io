...
2.  [2. 错误日志](#错误日志)
3.  [3. 慢查询日志](#慢查询日志)
    1.  [3.1. 启动慢查询日志](#启动慢查询日志)
    2.  [3.2. 设置阈值](#设置阈值)
    3.  [3.3. 参数log\_queries\_not\_using\_indexes](#参数log-queries-not-using-indexes)
    4.  [3.4. 将日志记录放入表中](#将日志记录放入表中)
    5.  [3.5. 慢查询日志分析工具](#慢查询日志分析工具)
        1.  [3.5.1. mysqldumpslow命令](#mysqldumpslow命令)
        2.  [3.5.2. pt-query-digest 工具](#pt-query-digest-工具)
4.  [4. 查询日志](#查询日志)
5.  [5. 二进制日志](#二进制日志)
    1.  [5.1. 开启二进制日志](#开启二进制日志)
    2.  [5.2. 查看二进制日志文件](#查看二进制日志文件)
    3.  [5.3. 二进制日志文件配置参数](#二进制日志文件配置参数)
...
### 参数log\_queries\_not\_using\_indexes

**另一个和慢查询日志有关的参数是 `log_queries_not_using_indexes`**,  
如果运行的SQL语句没有使用索引，则MySQL数据库同样会将这条SQL语句记录到慢查询日志文件。首先确认打开了log\_queries\_not\_using\_indexes;  
...
MySQL5.1开始可以将慢查询的日志记录放入一张表中，在mysql数据库下，名为slow\_log  
...
参数`log_output`指定了慢查询输出的格式，默认为file,可以将它设置成table，将变成了上面的slow\_log中  
...
更多关于 `mysqldumpslow` 命令的介绍，请参阅 ：[mysqldumpslow 的参数讲解与基础使用](https://github.com/luisedware/Archives/issues/7)
...
> 该工具可以将查询的剖析报告打印出来，可以分析结果输出到文件中，分析过程是先对查询语句的条件进行参数化，然后对参数化以后的查询进行分组统计，统计出各查询的执行时间，次数，占比等，可以借助分析结果找出问题进行优化。更多关于pt-query-digest的安装与使用，请参阅 : [http://www.ywnds.com/?p=8179](http://www.ywnds.com/?p=8179)
...
可以通过max\_binlog\_size参数来限定单个binlog文件的大小(默认1G)
...
*   [http://www.ywnds.com/?p=12839#comments](http://www.ywnds.com/?p=12839#comments)
